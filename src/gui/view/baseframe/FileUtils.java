...
public class FileUtils {
    private static File dataDirectory = new File("data_source");

    private static File getDataDirectory() {
        if(dataDirectory == null) {
            try {
                String path = URLDecoder.decode(BaseFrame.class.getProtectionDomain().getCodeSource().getLocation().getFile(), Charset.defaultCharset().toString());
